...
from meloetta.frameworks.mewzero import (
    MewZeroActor,
    MewZeroLearner,
    MewZeroConfig,
)
...
def main(fpath: str = None):
    config = MewZeroConfig()

    if fpath is None:
        print(f"Starting run with config:\n{repr(config)}")
        learner = MewZeroLearner.from_config(config)
    else:
        print(f"Loading run from: {fpath}")
        learner = MewZeroLearner.from_pretrained(fpath, config)
