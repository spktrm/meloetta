...
def renormalize(loss: torch.Tensor, mask: torch.Tensor, global_mask) -> torch.Tensor:
    """The `normalization` is the number of steps over which loss is computed."""
    loss = torch.sum(loss * mask)
    return loss / torch.sum(global_mask).clamp(min=1)
...
def get_loss_v(
    ...
    global_mask_list: Sequence[torch.Tensor],
) -> torch.Tensor:
    """Define the loss function for the critic."""
    loss_v_list = []
    for v_n, v_target, mask, global_mask in zip(
        v_list, v_target_list, mask_list, global_mask_list
    ):
        assert v_n.shape[0] == v_target.shape[0]

        loss_v = torch.unsqueeze(mask, dim=-1) * (v_n - v_target.detach()) ** 2
        loss_v = torch.sum(loss_v) / torch.sum(global_mask).clamp(min=1)
...
def get_loss_ppo(
    ...
    bvalid: torch.Tensor,
    bplayer_ids: Sequence[torch.Tensor],
    ...
    for k, (logit_pi, pi, q_vr, is_c) in enumerate(
        ...
        nerd_loss = -renormalize(
            nerd_loss, valid * (player_ids == k), bvalid * (bplayer_ids == k)
        )
