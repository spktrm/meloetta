...
class Loss(NamedTuple):
    ...
    action_type_policy_ent: float = 0
    move_policy_ent: float = 0
    max_move_policy_ent: float = 0
    switch_policy_ent: float = 0
    flag_policy_ent: float = 0
    target_policy_ent: float = 0
...
class Targets(NamedTuple):
    ...
    def slice(self, start: int, end: int, max_length: int = None) -> "Targets":
        ...
        for key, value in self._asdict().items():
            if value is not None:
                targets_dict[key] = value[:max_length, start:end].contiguous()
