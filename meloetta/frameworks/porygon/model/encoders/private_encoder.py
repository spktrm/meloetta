...
from meloetta.frameworks.porygon.model.encoders import private_spatial
...
class PrivateEncoder(nn.Module):
    def __init__(self, gen: int, n_active: int, config: config.PrivateEncoderConfig):
        ...
        move_embedding_dim = (
            ...
            + self.pp_sqrt_onehot.embedding_dim
        )
        entity_in = (
    ...
    def embed_moves(
        ...
        for i in range(4):
            move_slot[..., i] = i
        move_slot_emb = self.move_slot_onehot(move_slot)
    ...
    def forward(self, private_reserve: torch.Tensor):
        ...
        entity_embeddings = self.entity_out(entity_embeddings)
