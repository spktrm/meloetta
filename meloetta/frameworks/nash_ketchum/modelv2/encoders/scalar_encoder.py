...
class ScalarEncoder(nn.Module):
    def __init__(self, gen: int, n_active: int, config: config.ScalarEncoderConfig):
        ...
        self.boosts_mlp = nn.Sequential(
            linear_layer(len(BOOSTS), config.embedding_dim),
            MLP([config.embedding_dim, config.embedding_dim]),
        )
        self.volatiles_mlp = nn.Sequential(
            linear_layer(len(VOLATILES), config.embedding_dim),
            MLP([config.embedding_dim, config.embedding_dim]),
        )
        ...
        self.side_conditions_mlp = nn.Sequential(
            linear_layer(side_con_size, config.embedding_dim),
            MLP([config.embedding_dim, config.embedding_dim]),
        )
        ...
        layers = [
            VectorResblock(config.embedding_dim) for _ in range(config.num_resblocks)
        ]
        self.encoder = nn.Sequential(*layers)

        self.side_merge = VectorMerge(
            OrderedDict(
                boosts=config.embedding_dim,
                volatiles=config.embedding_dim,
                side_conditions=config.embedding_dim,
                switches=config.embedding_dim,
            ),
            config.embedding_dim,
        )
        self.public_side = MLP([2 * config.embedding_dim, config.embedding_dim])
    ...
    def forward(
        ...
        player_public_embedding: torch.Tensor,
        opponent_public_embedding: torch.Tensor,
    ) -> TensorDict:
        ...
        player_boosts, opponent_boosts = [
            t.squeeze(2) for t in self.boosts_mlp(boosts).chunk(2, 2)
        ]
        player_volatiles, opponent_volatiles = [
            t.squeeze(2) for t in self.volatiles_mlp(volatiles).chunk(2, 2)
        ]
        player_side_conditions, opponent_side_conditions = [
            t.squeeze(2) for t in self.side_conditions_mlp(side_conditions).chunk(2, 2)
        ]
        ...
        player_side = self.side_merge(
            OrderedDict(
                boosts=player_boosts,
                volatiles=player_volatiles,
                side_conditions=player_side_conditions,
                switches=player_public_embedding,
            )
        )
        opponent_side = self.side_merge(
            OrderedDict(
                boosts=opponent_boosts,
                volatiles=opponent_volatiles,
                side_conditions=opponent_side_conditions,
                switches=opponent_public_embedding,
            )
        )
        public_side = torch.cat((player_side, opponent_side), dim=-1)
        public_side = self.public_side(public_side)
