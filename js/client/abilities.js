...
*/
 ...
 const Abilities = {
	...
	battlebond: {
		onSourceAfterFaint(length, target, source, effect) {
			if (_optionalChain([effect, 'optionalAccess', _2 => _2.effectType]) !== 'Move') return;
	...
	beadsofruin: {
		...
		onAnyModifySpD(spd, target, source, move) {
			...
			if (!_optionalChain([move, 'access', _3 => _3.ruinedSpD, 'optionalAccess', _4 => _4.hasAbility, 'call', _5 => _5('Beads of Ruin')])) move.ruinedSpD = abilityHolder;
	...
	darkaura: {
		...
		onAnyBasePower(basePower, source, target, move) {
			...
			if (!_optionalChain([move, 'access', _7 => _7.auraBooster, 'optionalAccess', _8 => _8.hasAbility, 'call', _9 => _9('Dark Aura')])) move.auraBooster = this.effectState.target;
	...
	fairyaura: {
		...
		onAnyBasePower(basePower, source, target, move) {
			...
			if (!_optionalChain([move, 'access', _10 => _10.auraBooster, 'optionalAccess', _11 => _11.hasAbility, 'call', _12 => _12('Fairy Aura')])) move.auraBooster = this.effectState.target;
			if (move.auraBooster !== this.effectState.target) return;
			return this.chainModify([move.hasAuraBreak ? 3072 : 5448, 4096]);
		},
		name: "Fairy Aura",
	...
	galewings: {
		onModifyPriority(priority, pokemon, target, move) {
			if (_optionalChain([move, 'optionalAccess', _13 => _13.type]) === 'Flying' && pokemon.hp === pokemon.maxhp) return priority + 1;
		},
		name: "Gale Wings",
		rating: 2.5,
		num: 177,
	...
	iceface: {
		...
		onWeatherChange(pokemon, source, sourceEffect) {
			...
			if (_optionalChain([(sourceEffect ), 'optionalAccess', _14 => _14.suppressWeather])) return;
			if (!pokemon.hp) return;
			if (this.field.isWeather(['hail', 'snow']) &&
				pokemon.species.id === 'eiscuenoice' && !pokemon.transformed) {
				this.add('-activate', pokemon, 'ability: Ice Face');
	...
	naturalcure: {
		onCheckShow(pokemon) {
			...
			for (const curPoke of pokemon.side.active) {
				...
				if (!_optionalChain([curPoke, 'optionalAccess', _19 => _19.status])) {
	...
	opportunist: {
		onFoeAfterBoost(boost, target, source, effect) {
			if (_optionalChain([effect, 'optionalAccess', _20 => _20.fullname, 'optionalAccess', _21 => _21.endsWith, 'call', _22 => _22('Opportunist')]) || _optionalChain([effect, 'optionalAccess', _23 => _23.fullname, 'optionalAccess', _24 => _24.endsWith, 'call', _25 => _25('Mirror Herb')])) return;
	...
	owntempo: {
		...
		onHit(target, source, move) {
			if (_optionalChain([move, 'optionalAccess', _26 => _26.volatileStatus]) === 'confusion') {
	...
	pastelveil: {
		...
		onAllySetStatus(status, target, source, effect) {
			if (!['psn', 'tox'].includes(status.id)) return;
			if (_optionalChain([(effect ), 'optionalAccess', _28 => _28.status])) {
				const effectHolder = this.effectState.target;
	...
	pickpocket: {
		onAfterMoveSecondary(target, source, move) {
			if (source && source !== target && _optionalChain([move, 'optionalAccess', _29 => _29.flags, 'access', _30 => _30['contact']])) {
	...
	poisontouch: {
		...
		onModifyMove(move) {
			if (!_optionalChain([move, 'optionalAccess', _31 => _31.flags, 'access', _32 => _32['contact']]) || move.target === 'self') return;
			if (!move.secondaries) {
				move.secondaries = [];
			}
	...
	protosynthesis: {
		...
		onWeatherChange(pokemon) {
			...
			} else if (!_optionalChain([pokemon, 'access', _34 => _34.volatiles, 'access', _35 => _35['protosynthesis'], 'optionalAccess', _36 => _36.fromBooster])) {
	...
	purifyingsalt: {
		onSetStatus(status, target, source, effect) {
			if (_optionalChain([(effect ), 'optionalAccess', _37 => _37.status])) {
	...
	quarkdrive: {
		...
		onTerrainChange(pokemon) {
			...
			} else if (!_optionalChain([pokemon, 'access', _38 => _38.volatiles, 'access', _39 => _39['quarkdrive'], 'optionalAccess', _40 => _40.fromBooster])) {
	...
	serenegrace: {
		...
		onModifyMove(move) {
			...
			if (_optionalChain([move, 'access', _41 => _41.self, 'optionalAccess', _42 => _42.chance])) move.self.chance *= 2;
		},
		name: "Serene Grace",
		rating: 3.5,
		num: 32,
	...
	shieldsdown: {
		...
		onSetStatus(status, target, source, effect) {
			...
			if (_optionalChain([(effect ), 'optionalAccess', _43 => _43.status])) {
	...
	swordofruin: {
		...
		onAnyModifyDef(def, target, source, move) {
			const abilityHolder = this.effectState.target;
			if (abilityHolder === target) return;
			if (!_optionalChain([move, 'access', _44 => _44.ruinedDef, 'optionalAccess', _45 => _45.hasAbility, 'call', _46 => _46('Sword of Ruin')])) move.ruinedDef = abilityHolder;
	...
	tangledfeet: {
		...
		onModifyAccuracy(accuracy, target) {
			if (typeof accuracy !== 'number') return;
			if (_optionalChain([target, 'optionalAccess', _47 => _47.volatiles, 'access', _48 => _48['confusion']])) {
	...
	triage: {
		onModifyPriority(priority, pokemon, target, move) {
			if (_optionalChain([move, 'optionalAccess', _50 => _50.flags, 'access', _51 => _51['heal']])) return priority + 3;
		},
		name: "Triage",
		rating: 3.5,
		num: 205,
	...
	vitalspirit: {
		...
		onSetStatus(status, target, source, effect) {
			if (status.id !== 'slp') return;
			if (_optionalChain([(effect ), 'optionalAccess', _52 => _52.status])) {
	...
	waterbubble: {
		...
		onSetStatus(status, target, source, effect) {
			if (status.id !== 'brn') return;
			if (_optionalChain([(effect ), 'optionalAccess', _53 => _53.status])) {
