		...
		switch (cmd) {
			...
			case "-end": {
				...
				if (_id9 === "doomdesire" || _id9 === "futuresight") {
					...
					return (
						_line5 +
						_template17.replace("[TARGET]", this.pokemon(_pokemon11))
					);
				}
				var _templateId = "end";
				var _template16 = "";
			...
			case "-item": {
				...
				if (
					["thief", "covet", "bestow", "magician", "pickpocket"].includes(
						_id11
					)
				) {
			...
			case "-curestatus": {
				...
				if (!_template35)
					_template35 = this.template("end").replace("[EFFECT]", _status);
				return (
					_line11 +
			...
			case "-weather": {
				...
				if (BattleTextParser.effectId(kwArgs.from) === "orichalcumpulse") {
					return (
						_line13 +
						this.template("start", "orichalcumpulse").replace(
							"[POKEMON]",
			...
			case "-fail": {
				...
				if (
					...
					!["sunnyday", "raindance", "sandstorm", "hail", "snow"].includes(
						_id18
					)
				) {
