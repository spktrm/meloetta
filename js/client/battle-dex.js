		...
		this.species = {
			get: function (nameOrSpecies) {
				...
				if (window.BattleAliases && id in BattleAliases) {
					...
					for (
						var _i = 0, _BattleBaseSpeciesCha = BattleBaseSpeciesChart;
				...
				if (species.cosmeticFormes) {
					for (
						var _i2 = 0, _species$cosmeticForm = species.cosmeticFormes;
		...
		this.types = {
			...
			get: function (type) {
				if (!type || typeof type === "string") {
					...
					type =
						(window.BattleTypeChart && window.BattleTypeChart[id]) || {};
