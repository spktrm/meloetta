...
*/
 ...
 const Moves = {
	...
	gigatonhammer: {
		...
		flags: {protect: 1, mirror: 1},
		onDisableMove(pokemon) {
			if (_optionalChain([pokemon, 'access', _18 => _18.lastMove, 'optionalAccess', _19 => _19.id]) === 'gigatonhammer') pokemon.disableMove('gigatonhammer');
		},
		secondary: null,
		target: "normal",
		type: "Steel",
	...
	gmaxchistrike: {
		...
		condition: {
			...
			onStart(target, source, effect) {
				...
				if (!['costar', 'imposter', 'psychup', 'transform'].includes(_optionalChain([effect, 'optionalAccess', _20 => _20.id]))) {
			...
			onRestart(target, source, effect) {
				...
				if (!['costar', 'imposter', 'psychup', 'transform'].includes(_optionalChain([effect, 'optionalAccess', _21 => _21.id]))) {
					this.add('-start', target, 'move: G-Max Chi Strike');
				}
			},
			onModifyCritRatio(critRatio) {
	...
	grasspledge: {
		...
		onPrepareHit(target, source, move) {
			for (const action of this.queue.list ) {
				if (
					!action.move || !_optionalChain([action, 'access', _22 => _22.pokemon, 'optionalAccess', _23 => _23.isActive]) ||
					action.pokemon.fainted || action.maxMove || action.zmove
				) {
					continue;
				}
	...
	grassyterrain: {
		...
		condition: {
			...
			onFieldStart(field, source, effect) {
				if (_optionalChain([effect, 'optionalAccess', _26 => _26.effectType]) === 'Ability') {
	...
	gravity: {
		...
		condition: {
			duration: 5,
			durationCallback(source, effect) {
				if (_optionalChain([source, 'optionalAccess', _27 => _27.hasAbility, 'call', _28 => _28('persistent')])) {
	...
	healbell: {
		...
		onHit(target, source) {
			...
			let success = false;
			const allies = [...target.side.pokemon, ..._optionalChain([target, 'access', _29 => _29.side, 'access', _30 => _30.allySide, 'optionalAccess', _31 => _31.pokemon]) || []];
			for (const ally of allies) {
	...
	healblock: {
		...
		condition: {
			duration: 5,
			durationCallback(target, source, effect) {
				if (_optionalChain([source, 'optionalAccess', _32 => _32.hasAbility, 'call', _33 => _33('persistent')])) {
					this.add('-activate', source, 'ability: Persistent', effect);
					return 7;
				}
				return 5;
			...
			onTryHeal(damage, target, source, effect) {
				if ((_optionalChain([effect, 'optionalAccess', _34 => _34.id]) === 'zpower') || this.effectState.isZ) return damage;
				return false;
			},
	...
	hurricane: {
		...
		flags: {protect: 1, mirror: 1, distance: 1, wind: 1},
		onModifyMove(move, pokemon, target) {
			switch (_optionalChain([target, 'optionalAccess', _35 => _35.effectiveWeather, 'call', _36 => _36()])) {
			case 'raindance':
			case 'primordialsea':
				move.accuracy = true;
				break;
	...
	iceball: {
		...
		basePowerCallback(pokemon, target, move) {
			...
			if (_optionalChain([iceballData, 'optionalAccess', _37 => _37.hitCount])) {
	...
	magiccoat: {
		...
		condition: {
			...
			onStart(target, source, effect) {
				...
				if (_optionalChain([effect, 'optionalAccess', _40 => _40.effectType]) === 'Move') {
	...
	mirrormove: {
		...
		onTryHit(target, pokemon) {
			const move = target.lastMove;
			if (!_optionalChain([move, 'optionalAccess', _43 => _43.flags, 'access', _44 => _44['mirror']]) || move.isZ || move.isMax) {
				return false;
			}
	...
	psychicterrain: {
		...
		condition: {
			duration: 5,
			durationCallback(source, effect) {
				if (_optionalChain([source, 'optionalAccess', _48 => _48.hasItem, 'call', _49 => _49('terrainextender')])) {
					return 8;
				}
				return 5;
			},
			...
			onFieldStart(field, source, effect) {
				if (_optionalChain([effect, 'optionalAccess', _50 => _50.effectType]) === 'Ability') {
	...
	pursuit: {
		...
		onModifyMove(move, source, target) {
			if (_optionalChain([target, 'optionalAccess', _51 => _51.beingCalledBack]) || _optionalChain([target, 'optionalAccess', _52 => _52.switchFlag])) move.accuracy = true;
		},
		onTryHit(target, pokemon) {
	...
	ragingfury: {
		...
		onAfterMove(pokemon) {
			if (_optionalChain([pokemon, 'access', _53 => _53.volatiles, 'access', _54 => _54['lockedmove'], 'optionalAccess', _55 => _55.duration]) === 1) {
				pokemon.removeVolatile('lockedmove');
			}
		},
		secondary: null,
	...
	reflect: {
		...
		condition: {
			duration: 5,
			durationCallback(target, source, effect) {
				if (_optionalChain([source, 'optionalAccess', _56 => _56.hasItem, 'call', _57 => _57('lightclay')])) {
					return 8;
				}
				return 5;
			},
	...
	rollout: {
		...
		basePowerCallback(pokemon, target, move) {
			...
			if (_optionalChain([rolloutData, 'optionalAccess', _58 => _58.hitCount])) {
	...
	safeguard: {
		...
		condition: {
			duration: 5,
			durationCallback(target, source, effect) {
				if (_optionalChain([source, 'optionalAccess', _59 => _59.hasAbility, 'call', _60 => _60('persistent')])) {
					this.add('-activate', source, 'ability: Persistent', effect);
					return 7;
				}
				return 5;
	...
	shelltrap: {
		...
		onTryMove(pokemon) {
			if (!_optionalChain([pokemon, 'access', _61 => _61.volatiles, 'access', _62 => _62['shelltrap'], 'optionalAccess', _63 => _63.gotHit])) {
				this.attrLastMove('[still]');
	...
	spitup: {
		...
		basePower: 0,
		basePowerCallback(pokemon) {
			if (!_optionalChain([pokemon, 'access', _64 => _64.volatiles, 'access', _65 => _65['stockpile'], 'optionalAccess', _66 => _66.layers])) return false;
	...
	suckerpunch: {
		...
		onTry(source, target) {
			const action = this.queue.willMove(target);
			const move = _optionalChain([action, 'optionalAccess', _67 => _67.choice]) === 'move' ? action.move : null;
	...
	tailwind: {
		...
		condition: {
			duration: 4,
			durationCallback(target, source, effect) {
				if (_optionalChain([source, 'optionalAccess', _68 => _68.hasAbility, 'call', _69 => _69('persistent')])) {
					this.add('-activate', source, 'ability: Persistent', effect);
					return 6;
				}
				return 4;
	...
	thunder: {
		...
		flags: {protect: 1, mirror: 1},
		onModifyMove(move, pokemon, target) {
			switch (_optionalChain([target, 'optionalAccess', _70 => _70.effectiveWeather, 'call', _71 => _71()])) {
			case 'raindance':
			case 'primordialsea':
				move.accuracy = true;
				break;
	...
	torment: {
		...
		condition: {
			...
			onStart(pokemon, source, effect) {
				...
				if (_optionalChain([effect, 'optionalAccess', _72 => _72.id]) === 'gmaxmeltdown') this.effectState.duration = 3;
	...
	trickroom: {
		...
		condition: {
			duration: 5,
			durationCallback(source, effect) {
				if (_optionalChain([source, 'optionalAccess', _73 => _73.hasAbility, 'call', _74 => _74('persistent')])) {
					this.add('-activate', source, 'ability: Persistent', effect);
					return 7;
				}
				return 5;
		...
		condition: {
			...
			onModifyMove(move, pokemon) {
				if (move.secondaries && move.id !== 'secretpower') {
					...
					if (_optionalChain([move, 'access', _75 => _75.self, 'optionalAccess', _76 => _76.chance])) move.self.chance *= 2;
				}
			},
		},
		secondary: null,
	...
	wideguard: {
		...
		condition: {
			...
			onTryHit(target, source, move) {
				...
				if (_optionalChain([move, 'optionalAccess', _77 => _77.target]) !== 'allAdjacent' && move.target !== 'allAdjacentFoes') {
					return;
				}
				if (move.isZ || move.isMax) {
					if (['gmaxoneblow', 'gmaxrapidflow'].includes(move.id)) return;
	...
	wonderroom: {
		...
		condition: {
			duration: 5,
			durationCallback(source, effect) {
				if (_optionalChain([source, 'optionalAccess', _78 => _78.hasAbility, 'call', _79 => _79('persistent')])) {
					this.add('-activate', source, 'ability: Persistent', effect);
					return 7;
				}
				return 5;
